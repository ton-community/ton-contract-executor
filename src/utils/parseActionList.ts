import { Cell, loadCurrencyCollection, loadMessageRelaxed, Slice, MessageRelaxed, CurrencyCollection } from 'ton';
...
function parseActionsListImpl(actions: Slice|Cell): OutAction[] {
    ...
    if (actions instanceof Cell) {
        slice = actions.asSlice()
    } else {
        slice = actions
    }

    try {
        ref = slice.loadRef().asSlice()
    } catch (e) {
        return list
    }
    ...
    if (magic === 0x0ec3c86d) {
        outAction = {
            ...
            message: loadMessageRelaxed(slice.loadRef().asSlice())
        }
